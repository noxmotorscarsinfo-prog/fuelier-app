-- ========================================
-- 🥗 FUELIER - MIGRACIÓN COMPLETA
-- ========================================
-- Este script crea TODO desde cero:
-- - Elimina datos antiguos (opcional)
-- - Crea tabla base_meal_ingredients
-- - Inserta 33 ingredientes base
-- - Inserta 21 platos base
-- - Inserta 101 relaciones
-- ========================================

-- ========================================
-- PASO 1: LIMPIAR DATOS ANTIGUOS (OPCIONAL)
-- ========================================
-- IMPORTANTE: Descomenta estas líneas SOLO si quieres borrar todo y empezar de cero
-- CUIDADO: Esto borrará TODOS los datos de prueba que hayas creado

-- TRUNCATE TABLE base_meal_ingredients CASCADE;
-- DELETE FROM base_meals WHERE created_by IS NULL;
-- DELETE FROM base_ingredients WHERE created_by IS NULL;

-- ========================================
-- PASO 2: CREAR TABLA base_meal_ingredients
-- ========================================

-- Primero, eliminar la tabla si existe (para evitar conflictos)
DROP TABLE IF EXISTS base_meal_ingredients CASCADE;

-- Crear la tabla desde cero
CREATE TABLE base_meal_ingredients (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  meal_id text NOT NULL,
  ingredient_id text NOT NULL,
  amount_in_grams numeric NOT NULL CHECK (amount_in_grams > 0),
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  
  -- Foreign keys (se crean después de insertar los datos para evitar errores)
  CONSTRAINT fk_meal FOREIGN KEY (meal_id) REFERENCES base_meals(id) ON DELETE CASCADE,
  CONSTRAINT fk_ingredient FOREIGN KEY (ingredient_id) REFERENCES base_ingredients(id) ON DELETE CASCADE
);

-- Crear índices para mejor rendimiento
CREATE INDEX idx_base_meal_ingredients_meal_id ON base_meal_ingredients(meal_id);
CREATE INDEX idx_base_meal_ingredients_ingredient_id ON base_meal_ingredients(ingredient_id);

-- ========================================
-- PASO 3: INSERTAR INGREDIENTES BASE (33)
-- ========================================
...
-- ========================================
-- PASO 4: INSERTAR PLATOS BASE (21)
-- ========================================

-- DESAYUNOS (5)
INSERT INTO base_meals (id, name, type, variant, created_by) 
VALUES
...
-- ========================================
-- PASO 5: INSERTAR RELACIONES (101)
-- ========================================
...
SELECT 
  'Relaciones ingrediente-plato' as tabla,
  COUNT(*) as total 
FROM base_meal_ingredients;
